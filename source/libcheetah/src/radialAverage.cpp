...
void calculateRadialAverage(T *data2d, uint16_t *pixelmask2d, T *dataRadial, uint16_t *pixelmaskRadial, float * pix_r, long radial_nn, long pix_nn) {
	...
	uint16_t maskOutBits = PIXEL_IS_IN_PEAKMASK|PIXEL_IS_BAD|PIXEL_IS_HOT|PIXEL_IS_SATURATED|PIXEL_IS_INVALID|PIXEL_IS_DEAD|PIXEL_IS_TO_BE_IGNORED;
 ...
 *	Used for time-resolved WAXS analysis
 */
...
void addToRadialAverageStack(cEventData *eventData, cGlobal *global, int powderClass, int detIndex){
	...
	// (could be done outside of mutex, with risk of some confusion as to when full stacks are saved)
    for(long i=0; i<radial_nn; i++) {
