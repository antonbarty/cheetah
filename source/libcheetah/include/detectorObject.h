...
//#include "cheetah/release/pdsdata/cspad/ConfigV1.hh"
//#include "cheetah/release/pdsdata/cspad/ConfigV2.hh"
//#include "cheetah/release/pdsdata/cspad/ConfigV3.hh"
//#include "cheetah/cspad/CspadTemp.hh"
//#include "cheetah/cspad/CspadCorrector.hh"
//#include "cheetah/cspad/CspadGeometry.hh"
#ifndef DETECTOROBJECT_H
#define DETECTOROBJECT_H

#include <stdint.h>
#include "cheetah/dataVersion.h"
...
static const uint16_t PIXEL_IS_ALL = PIXEL_IS_INVALID | PIXEL_IS_SATURATED | PIXEL_IS_HOT | PIXEL_IS_DEAD | PIXEL_IS_SHADOWED | PIXEL_IS_IN_PEAKMASK
        | PIXEL_IS_TO_BE_IGNORED | PIXEL_IS_BAD | PIXEL_IS_OUT_OF_RESOLUTION_LIMITS | PIXEL_IS_MISSING | PIXEL_IS_NOISY | PIXEL_IS_ARTIFACT_CORRECTED
        | PIXEL_FAILED_ARTIFACT_CORRECTION | PIXEL_IS_PEAK_FOR_HITFINDER | PIXEL_IS_PHOTON_BACKGROUND_CORRECTED | PIXEL_IS_IN_JET;   // all bits

// for combined options
inline bool isAnyOfBitOptionsSet(uint16_t value, uint16_t option)
{
    return ((value & option) != 0);
}
inline bool isNoneOfBitOptionsSet(uint16_t value, uint16_t option)
{
    return ((value & option) == 0);
}
inline bool isAnyOfBitOptionsUnset(uint16_t value, uint16_t option)
{
    return ((value & option) != option);
}
inline bool isNoneOfBitOptionsUnset(uint16_t value, uint16_t option)
{
    return ((value & option) == option);
}
// for single options
inline bool isBitOptionSet(uint16_t value, uint16_t option)
{
    return isNoneOfBitOptionsUnset(value, option);
}
inline bool isBitOptionUnset(uint16_t value, uint16_t option)
{
    return isNoneOfBitOptionsSet(value, option);
}
